...
export default function DashboardPage() {
  ...
  if (authLoading || loading) {
    return <div className="min-h-screen flex items-center justify-center"><Loader2 size={32} className="animate-spin text-amber-500" /></div>;
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            ...
            <div className={`rounded-xl p-4 ${subscription ? "bg-amber-50 border border-amber-100" : "bg-slate-50 border border-slate-100"}`}>
              <div className="flex items-center gap-2 mb-2">
                <Star size={16} className={subscription ? "text-amber-500" : "text-slate-400"} />
              ...
              <Link href="/subscription" className="block text-center text-xs font-semibold bg-amber-500 text-white py-2 rounded-lg hover:bg-amber-600 transition-colors">
          ...
          {/* Alerts */}
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Bell size={16} className="text-amber-500" /> Alert Settings</h3>
            <div className="space-y-3">
              {[["Email Digest", true], ["WhatsApp Alerts", !!subscription], ["Price Alerts", true]].map(([label, enabled]) => (
                <div key={label} className="flex items-center justify-between text-sm">
                  <span className="text-slate-600">{label}</span>
                  <div className={`w-10 h-5 rounded-full relative cursor-pointer transition-colors ${enabled ? "bg-amber-500" : "bg-slate-200"}`}>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><Bookmark size={16} className="text-amber-500" /> Recent Articles</h3>
              <Link href="/news" className="text-xs text-amber-600 font-semibold hover:underline">Browse More</Link>
            </div>
            <div className="space-y-3">
              {articles.map((article) => (
                <Link key={article._id || article.id} href={`/article/${article._id || article.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-800 line-clamp-1 group-hover:text-amber-600 transition-colors">{article.title}</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><Download size={16} className="text-amber-500" /> Magazines</h3>
              <Link href="/magazines" className="text-xs text-amber-600 font-semibold hover:underline">All Issues</Link>
            </div>
            <div className="space-y-3">
              {magazines.map((mag) => (
                <div key={mag._id || mag.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                  ...
                  {(mag.accessType !== "premium" || subscription) && mag.fileUrl ? (
                    <a href={mag.fileUrl} target="_blank" rel="noreferrer"
                      className="shrink-0 p-2 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors">
                      <Download size={14} />
                    </a>
                  ) : (
                    <Link href="/subscription" className="shrink-0 p-2 bg-slate-100 text-slate-400 rounded-lg hover:bg-amber-50 hover:text-amber-600 transition-colors">
                      <Download size={14} />
                    </Link>
                  )}
                </div>
