  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[["Premium Users", "1,243", "bg-amber-50 border-amber-100 text-amber-700"], ["Basic Users", "892", "bg-blue-50 border-blue-100 text-blue-700"], ["Free Users", "2,686", "bg-slate-50 border-slate-200 text-slate-600"]].map(([label, val, cls]) => (
            ...
            <tbody>
              {subscriptions.map((sub) => (
                <tr key={sub.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-4">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${sub.plan === "Premium" ? "bg-amber-100 text-amber-700" : sub.plan === "Basic" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"}`}>
                      {sub.plan}
                    </span>
                  </td>
