    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-8">
        {[
          ...
          { label: "Revenue", value: `Rs ${((s.totalRevenue || s.revenue || 0) / 100000).toFixed(1)}L`, icon: TrendingUp, color: "text-amber-600 bg-amber-50" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/admin/users" className="text-xs text-amber-600 font-semibold hover:underline">View All</a>
          </div>
          {users.length === 0 ? (
            ...
            <div className="space-y-3">
              {users.map((u) => (
                <div key={u._id} className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-amber-100 rounded-full flex items-center justify-center text-xs font-bold text-amber-700 shrink-0">
                    {u.name?.[0] || "U"}
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/admin/articles" className="text-xs text-amber-600 font-semibold hover:underline">Manage</a>
        </div>
            ...
            <tbody>
              {articles.map((a) => (
                <tr key={a._id || a.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-4 py-3"><span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full">{a.category}</span></td>
                  <td className="px-4 py-3"><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${a.premium || a.accessType === "premium" ? "bg-slate-900 text-amber-400" : "bg-green-100 text-green-700"}`}>{a.premium || a.accessType === "premium" ? "Premium" : "Free"}</span></td>
