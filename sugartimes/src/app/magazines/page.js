    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      {latest && (
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 mb-10 flex flex-col md:flex-row gap-6 items-center">
          <img src={latest.cover || latest.coverImage || "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400"}
            alt={latest.title} className="w-40 h-52 object-cover rounded-xl shadow-2xl" />
          <div className="text-white">
            <span className="bg-amber-500 text-white text-xs font-bold px-3 py-1 rounded-full">Latest Issue</span>
        ...
        {magazines.map((mag) => (
          <div key={mag._id || mag.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-md transition-all group">
            <div className="relative h-56 overflow-hidden">
              <img src={mag.cover || mag.coverImage || "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400"}
              ...
              <div className="absolute top-2 right-2">
                {isPremium(mag)
                  ? <span className="bg-amber-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1"><Lock size={9} /> Premium</span>
            ...
            <div className="p-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => setPreview(mag)}
                  className="flex-1 flex items-center justify-center gap-1 text-xs font-semibold py-2 border border-slate-200 rounded-lg hover:border-amber-400 hover:text-amber-600 transition-colors">
                  <Eye size={12} /> Preview
                </button>
                {canAccess(mag)
                  ...
                  : <Link href="/subscription"
                      className="flex-1 flex items-center justify-center gap-1 text-xs font-semibold py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-4 mb-4">
              <img src={preview.cover || preview.coverImage || "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400"}
              ...
              <div>
                ...
                {isPremium(preview) && <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full">Premium Issue</span>}
              </div>
            </div>
            ...
            {canAccess(preview)
              ...
              : <Link href="/subscription" className="block w-full text-center bg-amber-500 hover:bg-amber-600 text-white font-semibold py-3 rounded-xl transition-colors">
