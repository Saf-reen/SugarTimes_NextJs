      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2 flex-wrap mb-6">
            {categories.map((cat) => (
              <button key={cat} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${cat === "All" ? "bg-amber-500 text-white" : "bg-white border border-slate-200 text-slate-600 hover:border-amber-400"}`}>
                {cat}
              </button>
            ))}
          </div>
              ...
              <div key={policy.id} className="bg-white rounded-2xl border border-slate-100 p-6 hover:shadow-sm transition-all group">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-amber-600 transition-colors">{policy.title}</h3>
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 flex gap-3">
                  <button className="text-sm text-amber-600 font-semibold hover:underline flex items-center gap-1">
        ...
        <aside className="hidden lg:block w-72 shrink-0 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5">
            ...
            <button className="w-full bg-amber-500 text-white text-sm font-semibold py-2 rounded-lg hover:bg-amber-600 transition-colors">Enable Alerts</button>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-5">
            ...
            <ul className="space-y-2 text-sm text-slate-600">
              {["Ministry of Food", "Ministry of Agriculture", "Ministry of Petroleum", "CCEA", "DGFT", "NITI Aayog"].map((m) => (
                <li key={m} className="hover:text-amber-600 cursor-pointer transition-colors flex items-center gap-2">
