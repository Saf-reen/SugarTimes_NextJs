    ...
    <Link href={`/article/${article.id}`} className="group block bg-white rounded-xl border border-slate-100 overflow-hidden hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
      <div className={`relative overflow-hidden ${compact ? "h-36" : "h-48"}`}>
        ...
        <div className="absolute top-2 left-2 flex gap-1.5">
          ...
          {article.premium && (
            <span className="bg-slate-900 text-amber-400 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
              <Lock size={9} /> Premium
            </span>
          )}
        </div>
      ...
      <div className="p-4">
        <h3 className={`font-semibold text-slate-800 group-hover:text-amber-600 transition-colors leading-snug mb-2 ${compact ? "text-sm line-clamp-2" : "text-base line-clamp-2"}`}>
          {article.title}
        </h3>
