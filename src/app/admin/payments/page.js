...
const statusIcon = { Success: <CheckCircle size={14} className="text-green-500" />, Pending: <Clock size={14} className="text-green-500" />, Failed: <XCircle size={14} className="text-red-500" /> };
const statusClass = { Success: "bg-green-100 text-green-700", Pending: "bg-green-100 text-green-700", Failed: "bg-red-100 text-red-600" };
  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[["Total Revenue", `₹${total.toLocaleString()}`, "text-green-600 bg-green-50"], ["Pending", payments.filter((p) => p.status === "Pending").length, "text-green-600 bg-green-50"], ["Failed", payments.filter((p) => p.status === "Failed").length, "text-red-600 bg-red-50"]].map(([label, val, cls]) => (
            ...
            <tbody>
              {payments.map((p) => (
                <tr key={p.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-4"><span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-0.5 rounded-full">{p.plan}</span></td>
