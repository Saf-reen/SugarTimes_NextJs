    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={fetchMarkets} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-green-600 transition-colors">
          ...
          <div className="flex gap-2 overflow-x-auto">
            {states.map((s) => (
              <button key={s} onClick={() => setFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${filter === s ? "bg-green-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-green-50"}`}>
                {s}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((row, i) => (
                  <tr key={row._id || i} className="border-t border-slate-50 hover:bg-green-50/40 transition-colors">
