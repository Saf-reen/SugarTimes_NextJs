      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="space-y-12">
            {/* Featured Video */}
            <section className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100 p-4">
               ...
               <div className="p-6">
                  <span className="bg-green-100 text-green-700 text-[10px] font-black uppercase px-2 py-0.5 rounded-full mb-3 inline-block">NOW PLAYING: {selectedVideo?.category}</span>
                  <h2 className="text-2xl font-black text-slate-900">{selectedVideo?.title}</h2>
               </div>
            </section>
              ...
              {videos.map((v) => (
                ...
                >
                  <div className="relative aspect-video rounded-2xl overflow-hidden mb-3 bg-slate-200">
                    <img 
                       src={`https://img.youtube.com/vi/${v.videoUrl.split("v=")[1]?.split("&")[0] || v.videoUrl.split("/").pop()}/mqdefault.jpg`} 
                       alt={v.title} 
